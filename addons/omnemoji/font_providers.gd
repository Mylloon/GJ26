...
static func _load_json_providers(filename: String) -> Dictionary:
	...
	if error != OK:
		push_error(
			(
				"OmnEmoji: Failed to parse JSON in %s: %s (line %d)"
...
static func get_installed_fonts_summary() -> String:
	...
	for id in _emoji_providers:
		...
		if size > 0:
			lines.append(
				"  [%s] %s%s (%.2f MB)" % [installed, name, recommended, size / (1024.0 * 1024.0)]
			)
		else:
...
static func format_provider_info(provider_id: String, is_emoji := true) -> String:
	var provider: Dictionary = (
		get_emoji_provider(provider_id) if is_emoji else get_text_provider(provider_id)
	)
	if provider.is_empty():
