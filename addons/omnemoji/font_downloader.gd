...
func _extract_font_from_zip(zip_data: PackedByteArray) -> bool:
	...
	if written_size != font_data.size():
		print(
			(
				"[OmnEmoji] ✗ Write verification failed: expected %d bytes, got %d"
				% [font_data.size(), written_size]
			)
		)
		return false
...
func is_downloading() -> bool:
	return (
		_http_request != null
		and _http_request.get_http_client_status() != HTTPClient.STATUS_DISCONNECTED
	)
