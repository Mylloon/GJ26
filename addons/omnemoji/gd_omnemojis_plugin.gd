...
func _enter_tree() -> void:
	...
	if not _verify_fonts_exist():
		if _is_auto_download_enabled():
			...
			push_error(
				"[OmnEmoji] Required fonts missing! Enable auto-download or configure manually."
			)
...
func _register_project_settings() -> void:
	...
	_add_setting(SETTING_ENABLED, true, TYPE_BOOL, PROPERTY_HINT_NONE, "")

	# 2. Emoji Provider - Primary emoji font selection
	_add_setting(SETTING_EMOJI_PROVIDER, 2, TYPE_INT, PROPERTY_HINT_ENUM, ",".join(emoji_options))  # Default to first provider (noto)

	# 3. Text Provider - Primary text font selection
	_add_setting(SETTING_TEXT_PROVIDER, 2, TYPE_INT, PROPERTY_HINT_ENUM, ",".join(text_options))  # Default to first provider (noto_sans)

	# 4. Auto Download - Download missing fonts automatically
	_add_setting(SETTING_AUTO_DOWNLOAD, true, TYPE_BOOL, PROPERTY_HINT_NONE, "")

	# 5. Custom Emoji Font - Path (only used when provider = Custom)
	_add_setting(
		SETTING_EMOJI_FONT, "", TYPE_STRING, PROPERTY_HINT_FILE, "*.ttf,*.otf,*.woff,*.woff2"
	)

	# 6. Custom Text Font - Path (only used when provider = Custom)
	_add_setting(
		SETTING_TEXT_FONT, "", TYPE_STRING, PROPERTY_HINT_FILE, "*.ttf,*.otf,*.woff,*.woff2"
	)
...
func _add_setting(
	path: String, default_value: Variant, type: int, hint: int, hint_string: String
) -> void:
...
func _create_font_resource(ttf_path: String, resource_path: String, font_type: String) -> void:
	...
	if err != OK:
		push_error(
			"[OmnEmoji] Failed to create %s font resource: %s" % [font_type, error_string(err)]
		)
	else:
...
func _apply_emoji_fallback() -> void:
	...
	if using_system_text:
		...
		if (
			current_font_path != ""
			and current_font_path != MERGED_FONT_RESOURCE
			and ResourceLoader.exists(current_font_path)
		):
...
func _build_merged_font(main_font_path: String, emoji_font: FontFile) -> FontVariation:
	...
	if (
		main_font_path != ""
		and main_font_path != MERGED_FONT_RESOURCE
		and ResourceLoader.exists(main_font_path)
	):
...
func get_font_config() -> Dictionary:
	...
	return {
		...
		"emoji_font_installed":
		OmnEmojiFontProviders.is_emoji_font_installed(emoji_id) if emoji_id else true,
		"text_provider_id": text_id,
		"text_provider_name": text_provider.get("name", "Custom/System"),
		"text_font_path": _get_text_font_path(),
		"text_font_installed":
		OmnEmojiFontProviders.is_text_font_installed(text_id) if text_id else true,
